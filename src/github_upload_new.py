...
"""
Kan Hücresi Tespit Projesi - GitHub Yükleme Betiği (Yeni Token)
Bu betik, projenin tüm dosyalarını yeni bir GitHub deposuna yükler.
"""
...
GITHUB_TOKEN = os.environ.get('GITHUB_TOKEN', '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')
GITHUB_USERNAME = 'burakkurt07'
REPO_NAME = 'blood-cell-detection-project'
...
def setup_git_repo():
    ...
    # Uzak depoyu ekle (token'ı URL'de kullanmadan)
...
def push_to_github():
    ...
    # GitHub'a gönder (token'ı komut satırında kullan)
    push_cmd = f'git -c credential.helper= -c core.askPass= push -u origin master'
    ...
    try:
        # Subprocess ile çalıştır ve ortam değişkenlerini aktar
        ...
        if process.returncode != 0:
            ...
            # Alternatif yöntem: Doğrudan URL'de token kullan
            alt_push_cmd = f'git push -u https://{GITHUB_TOKEN}@github.com/{GITHUB_USERNAME}/{REPO_NAME}.git master'
            result = run_command(alt_push_cmd)
        ...
        print(f"Push işlemi sırasında bir hata oluştu: {e}")
        return False
...
def main():
    ...
    if not create_github_repo():
        print("GitHub deposu oluşturulamadı, işlem durduruluyor.")
        return False
    ...
    with open(os.path.join(PROJECT_DIR, 'time_tracking.md'), 'a') as f:
        f.write(f"- GitHub'a yükleme (yeni depo): Başlangıç - {time.strftime('%d Nisan %Y %H:%M:%S')}, Bitiş - {time.strftime('%d Nisan %Y %H:%M:%S', time.localtime(end_time))}, Süre - {github_time:.2f} saniye\n")
